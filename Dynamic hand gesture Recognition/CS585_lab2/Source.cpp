...
int main()
{
	...
	while (1)
	{
		...
		//blur(frame0, frame0, cv::Size(7, 7));
		//----------------
		...
		for (int x = 0; x< myMH.rows; x++){
			...
			if (i > 350) {
				cout << "horizontal\n";
				putText(frame0, "Hand wave detected", cvPoint(100, 100), FONT_HERSHEY_PLAIN,
					3, cvScalar(0, 255, 0), 3, 8);
				break;
			}
		}
...
void mySkinDetect(Mat& src, Mat& dst) {
	...
	////Init a zero mat, CV_8UC1 is 1 channal, black white.
